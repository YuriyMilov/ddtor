...
class TextEntry extends Column implements ActionListener {
	...
	private TextEntry(String text, ACEEditor owner, Map<String,ResultItem> resultItemsTemplate,
		...
		sentenceButton.setFont(new Font(Styleqq.fontTypeface, Font.PLAIN, new Extent(13)));
	...
	public void setSelected(boolean selected) {
		...
		if (comment) {
			sentenceButton.setFont(new Font(Styleqq.fontTypeface, Font.ITALIC, new Extent(13)));
		} else {
			sentenceButton.setFont(new Font(Styleqq.fontTypeface, Font.PLAIN, new Extent(13)));
		}
		if (selected) {
			sentenceButton.setBackground(Styleqq.mediumBackground);
			sentenceButton.setRolloverBackground(Styleqq.mediumBackground);
			if (error) {
			...
			sentenceButton.setRolloverBackground(Styleqq.lightBackground);
			if (error) {
				sentenceButton.setForeground(Color.RED);
				sentenceButton.setRolloverForeground(Color.RED);
			} else if (comment) {
	...
	public void setExpanded(boolean expanded) {
		...
		this.expanded = expanded;
		if (expanded) {
			expandButton.setIcon(Styleqq.getImage(imgpath + "down.png"));
			add(resultsColumn);
		} else {
			expandButton.setIcon(Styleqq.getImage(imgpath + "right.png"));
