...
class MenuBlock extends Column implements ActionListener {
	...
	MenuBlock(int width, int height, int colorShift, ActionListener actionListener) {
		...
		label.setDisabledFont(new Font(Styleqq.fontTypeface, Font.ITALIC, new Extent(11)));
		...
		enlargeButton.setFont(new Font(Styleqq.fontTypeface, Font.BOLD, new Extent(11)));
		enlargeButton.setForeground(Styleqq.lightBackground);
		...
		container.setBackground(shiftColor(Styleqq.lightBackground));
		...
		menuBaseColumn.setBackground(shiftColor(Styleqq.mediumBackground));
		menuBaseColumn.add(menuColumn);
		container.add(menuBaseColumn);
	}
	
	private Color shiftColor(Color color) {
		return Styleqq.shiftColor(color, colorShift);
	}
