...
public class WordEditorWindow extends WindowPane implements TabSelectionListener {
	...
	public WordEditorWindow(String title, String type, int width, int height) {
		...
		setModal(true);
		setTitle(title);
		setTitleFont(new Font(Styleqq.fontTypeface, Font.ITALIC, new Extent(13)));
		setWidth(new Extent(width));
		setHeight(new Extent(height));
		setResizable(false);
		setTitleBackground(Styleqq.windowTitleBackground);
		...
		tabPane.setFont(new Font(Styleqq.fontTypeface, Font.ITALIC, new Extent(13)));
		tabPane.setTabActiveForeground(Color.BLACK);
		tabPane.setTabInactiveBackground(Styleqq.shadedBackground);
		tabPane.setTabInactiveForeground(Styleqq.darkDisabled);
		tabPane.setTabRolloverEnabled(true);
		tabPane.setTabRolloverForeground(Color.BLACK);
		tabPane.setTabRolloverBackground(Styleqq.lightBackground);
