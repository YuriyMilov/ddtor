...
import ch.uzh.ifi.attempto.echocomp.Styleqq;
...
public abstract class MenuItem extends Button {
	...
	public MenuItem(String menuGroup) {
		...
		setBackground(Styleqq.mediumBackground);
		setForeground(Styleqq.darkForeground);
		setRolloverEnabled(true);
		setRolloverForeground(Styleqq.lightForeground);
		setRolloverBackground(Styleqq.darkBackground);
        setLineWrap(false);
        updateStyle();
	}
    ...
    public void setColorShift(int colorShift) {
    	...
    	setBackground(Styleqq.shiftColor(super.getBackground(), colorShiftDiff));
    	setForeground(Styleqq.shiftColor(super.getForeground(), colorShiftDiff));
    	setRolloverBackground(Styleqq.shiftColor(super.getRolloverBackground(), colorShiftDiff));
    	setRolloverForeground(Styleqq.shiftColor(super.getRolloverForeground(), colorShiftDiff));
    }
	...
	protected void updateStyle() {
		if (highlighted) {
			setFont(new Font(Styleqq.fontTypeface, Font.BOLD, new Extent(12)));
		} else {
			setFont(new Font(Styleqq.fontTypeface, Font.PLAIN, new Extent(12)));
		}
	}
