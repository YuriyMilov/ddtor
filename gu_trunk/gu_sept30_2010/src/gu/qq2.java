...
//System.out.println(s);
		...
		// /////////////// adding work order
		/*
		 * int t = 9999; String[] s4=null; try { String str =
		 * req.getParameter("a"); String id = req.getParameter("id");
		 * 
		 ...
		 * 
		 * 
		 * if (words == null) s4 = new String[] { "r", "50.0", "-60.0",
		 * "address","2", "text" }; else s4 = words;
		 * 
		 * List<Mrkr4> rd = (List<Mrkr4>) pm.newQuery("SELECT FROM " +
		 * Mrkr4.class.getName()+ " WHERE s5==\""+id+"\"").execute(); for (int i
		 * = 0; i < rd.size(); i++) { Mrkr4 mr = pm.getObjectById(Mrkr4.class,
		 * rd.get(i).getId()); pm.deletePersistent(mr); } for (int i = 0; i <
		 * s4.length; i = i + 6) { Mrkr4 ns = new Mrkr4(new Date(), s4[i], s4[i
		 * + 1], s4[i + 2], s4[i + 3], s4[i + 4], s4[i + 5]);
		 ...
		 * (rr.size() == 0) { Mrkr4 ns = new Mrkr4(new Date(), s4[i], s4[i + 1],
		 ...
		 * 
		 * ns = new Mrkr4(new Date(), s4[i], s4[i + 1], s4[i + 2], s4[i + 3],
		 * s4[i + 4], s4[i + 5], user); pm.makePersistent(ns); }
		 * 
		 * 
		 * }
		 * 
		 * 
		 * } catch (Exception eee) { s=eee.toString(); }
		 */
