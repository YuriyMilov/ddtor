...
public class sta {
	...
	public static void wtf(String str, String furl) {
		...
		try {
			...
			Writer out = new OutputStreamWriter(fos);// UTF-8 CP1251
			out.write(str);
			out.close();
		} catch (IOException e) {
			e.printStackTrace();
	...
	public static void wtf_ru(String str, String furl, String enc) {
		...
		try {
			FileOutputStream fos = new FileOutputStream(furl);
			Writer out = new OutputStreamWriter(fos, enc);// UTF-8 CP1251
			out.write(str);
			out.close();
		} catch (IOException e) {
			e.printStackTrace();
	...
	public static void wtf_rut(String str, String furl) {
		...
		try {
			FileOutputStream fos = new FileOutputStream(furl,true);
			Writer out = new OutputStreamWriter(fos, "CP1251");// UTF-8 CP1251
			out.write(str+"\r\n");
			out.close();
		} catch (IOException e) {
			e.printStackTrace();
	...
	public static String rff(String s) throws Exception {
		return rfu2("file:///" + System.getProperties().getProperty("user.dir")
				+ "/" + s, "UTF8");
	}
	public static String rffw(String s) throws Exception {
		return rfu("file:///" + System.getProperties().getProperty("user.dir")
				+ "/" + s);
	}
	...
	public static void wtf_utf(String str, String furl) {
		...
		try {
			FileOutputStream fos = new FileOutputStream(furl);
			Writer out = new OutputStreamWriter(fos, "UTF-8");// UTF-8 CP1251
			out.write(str);
			out.close();
		} catch (IOException e) {
			e.printStackTrace();
	...
	public static String getCookie(String host, String page) throws Exception {
		...
		while ((s = br.readLine()) != null) {
			...
			if (s.indexOf("Cookie:") > -1)
				c = s.substring(s.indexOf("Cookie:"));
			if (s.indexOf("</HTML>") > -1)
				break;
		}
		inStream.close();
	...
	public static void list() {
		for (int i = 11; i < 23; i++)
			for (int j = 11; j < 44; j++)
				wtft("k" + i + "-k-" + j + " " + "k" + i, "list.txt");
		for (int i = 983; i < 999; i++)
			for (int j = 101; j < 134; j++)
				wtft("k" + i + "-" + j + " " + "k" + i, "list.txt");
	}
	...
	public static void bin(String blog, String s) {
		try {
			...
			myService.setUserCredentials(blog.substring(0, 3)
					+ "@quicklydone.com", "tverskoy");
	  ...
	  public static String rsp(String s){
		...
		public static void zi(String s, String z, int i) throws Exception {
			// sta.rfu_win("http://www.mirslov.ru/7/1/451447/");
...
//			Cookie: PREF=ID=b458ea7314033b74:TM=1178197060:LM=1178197060:S=GngbPwtqZNAVfRxa; SID=DQAAAHAAAADnJlIc4LQ1qXPa05WaYxjgCABnnw3SNAX_Dl35y-obXP9lZizdM1t0KBWPx8Us7OamjODExHuB_BLDPDEmR4VYvJbnvTgR8MBqtN9-CDbPYuRWGQb5Ykshe5idCblAWJsd5S6VZvFKLcpi0tv2Nzac
