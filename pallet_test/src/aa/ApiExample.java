...
public class ApiExample {
  ...
  public static void main(String[] args) throws ServiceException, IOException {
	  ...
	  args =new String[]{"kuka@feofan.com","90909000"};
	  if (args.length == 0) {
    ...
    example.runUpdate("insert into 2065823 (Text, Location) values " +
        "('mona lisa', 'michelangelo')");
    // To correct the entry, we first retrieve its row ID.
   // example.runSelect("select rowid from 2065823 where painting = 'mona lisa'");
    // Update the entry. It is assumed that the previous statement
    // returned a row ID of 25.
   // example.runUpdate("update 2065823 set artist = 'da vinci' where rowid = '25'");
    // The entry is deleted.
   // example.runUpdate("delete from 2065823 where rowid = '25'");
  }
}